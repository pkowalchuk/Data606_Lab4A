...
The distribution seems to be somewhat normal, although slightly right skewed. The histogram plot shows good resemblance with the normal distribution, although we see the slight right skew. Similar to the the q-q plot in which the center points fall nicely on the line, but we also see point towards the right edge diverging from the line. 

```{r}
h<-hist(area,freq = FALSE,xlim = c(0,4000))
...
We see a similar distribution in the sample compared to the population. The mean and standard deviations are also somewhat similar. In the q-q plot we do see how the sample doesn't fall as nicely on the line as does the population. The distribution also doesn't show as nicely normal, but also we don't see it being skewed to the right so much. Every run of the sample does show slightly different results though.  In the graph below, the blue line is our population distribution, while the red line is our sample distribution. The histogram shown is for the sample.

```{r}
h<-hist(samp1,freq = FALSE,xlim=c(0,4000),ylim=c(0,0.001))
z <- dnorm(x,mean(samp1),sd(samp1))
lines(x = x, y = y, col = "blue")
lines(x = x, y = z, col = "red")
legend(2500, 0.0007, legend=c("Sample", "Population"),
...
We should not expect to get the same mean each time we sample. A second sample could be closer, or not, to the population mean. As we increase our sample size we would expect the mean of each larger one to be closer to the population mean. But even then we should not be surprised to find a given larger sample to show a mean further away from the population mean. One other fact to consider is that increasing the sample size might make each observation non-independent. Rule of thumb is that samples should be less that 10% of the population. Our sample of 1000 is actually larger than 10% of the population.
...
We should have 5000 elements in the vector sample_means50, that's the number of samples of 50 we took. The resulting distribution is normal with a center, or mean, very close to the population mean of 1499.69. If we take 50000 samples of 50, we also get a normal distribution which can be centered closer to the population mean of 1499.69, but which we shouldn't be surprised to find not being exactly the same.   
...
```{r}
sample_means50_50000 <- rep(NA, 50000)
...
h<-hist(sample_means50_50000,freq = FALSE,xlim = c(1200,1800),ylim=c(0,0.006))
x <- seq(1200,1800, by = 0.01)
y <- dnorm(x,mean(sample_means50_50000),sd(sample_means50_50000))
lines(x = x, y = y, col = "blue")

mean(sample_means50_50000)
```
...
sample_means_small should have 100 elements. Each element represents the mean of 50 samples taken from the population.
...
To answer this we calculate the means and the standard deviations of our vectors with sample means. As we can see, the center or mean of them does not change a lot and stays close to the population mean. But the spread does change a lot when the number of samples used for each sample mean increases. As the number of samples increases, the spread or standard deviation decreases.
...
Our best estimate for the population mean is the mean of our sample, or this one sample mean.

```{r}
sample50_price <- sample(price, 50)
mean(sample50_price)
```
...
As seen below the distribution is normal. Our best guess for the mean home price of the population is the mean of our sample means. Below we compare this number with the actual mean of the population.
...
h<-hist(sample_means50,freq = FALSE)
x <- seq(0,240000, by = 0.01)
y <- dnorm(x,mean(sample_means50),sd(sample_means50))
lines(x = x, y = y, col = "blue")
...
The distribution is once again normal centered at about the same value, but this distribution of sample means of 150 samples it is much narrower. The best guess of the mean sale price of the population is the mean of these sample means, calculated below.
...
h<-hist(sample_means150,freq = FALSE,ylim=c(0,0.00008))
x <- seq(0,240000, by = 0.01)
y <- dnorm(x,mean(sample_means150),sd(sample_means150))
lines(x = x, y = y, col = "blue")
...
The distribution in 3, the one with sample means calculated from more sample, is narrower. Is concerned with getting values that are more often closer to the true value we would prefer a narrower distribution, one with a smaller spread.
